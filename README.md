...
[![MercuryCore Tests](https://github.com/angel12/mercurychat/actions/workflows/mercurycore-tests.yml/badge.svg)](https://github.com/angel12/mercurychat/actions/workflows/mercurycore-tests.yml)
[![App Tests](https://github.com/angel12/mercurychat/actions/workflows/app-tests.yml/badge.svg)](https://github.com/angel12/mercurychat/actions/workflows/app-tests.yml)
...
Mercury Chat has no backend of its own — see [PRIVACY.md](PRIVACY.md) for what the app stores and where it sends it. The same policy is published at <https://angel12.github.io/mercurychat/>.
...
GitHub Actions runs both suites on every PR targeting `main`, every push to `main`, and on manual dispatch, on GitHub-hosted **macOS** runners (`macos-15`) — the tests exercise Network.framework and the Keychain, so they cannot build on Linux runners. Each workflow cancels an in-progress run for the same ref when a newer one starts.

- `.github/workflows/mercurycore-tests.yml` — the MercuryCore suite (`swift test --package-path Packages/MercuryCore`).
- `.github/workflows/app-tests.yml` — the app-layer `MercuryTests` bundle, driven by `xcodebuild` because `swift test` cannot see an Xcode target. It first checks that the committed `Mercury.xcodeproj` still lists every file in `MercuryTests/`: the project is generated by XcodeGen but committed, and CI does not run `xcodegen`, so a test file added without regenerating would be absent from the bundle and the suite would pass green having never compiled it.
- `.github/workflows/pages.yml` — not a test job: on a push to `main` that touches the policy page, it publishes only `docs/index.html` (the privacy policy) and its icon to GitHub Pages at <https://angel12.github.io/mercurychat/>.

Both test workflows fetch MercuryKit from GitHub.
...
Live checks below were run against `hermes serve` 0.20.0 in token mode on 2026-08-11 unless noted. The client has since adopted newer `hermes serve` surfaces and now requires desktop contract 7; changes not re-verified live below are covered by the automated suites.
...
- [x] `swift test` green — now enforced by CI on every PR (historical count at Milestone 1: 199 tests — 110 MercuryKitTests + 89 ChatCoreTests: endpoint parsing, credentials/cookie extraction, payload wrappers, PKCE vectors + live loopback-listener round trips, real-socket networking against loopback HTTP/WebSocket test servers, transcript reducer + hydration, attachment markers + upload sequencing).
- [x] App-layer suite (`MercuryTests` Xcode target, AppModel glue) green — enforced by CI on every PR (historical count at Milestone 1: 51 tests). Not run by `swift test`; run it locally with `xcodebuild test -scheme MercuryTests -destination 'platform=macOS'`.
